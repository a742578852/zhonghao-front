	...
	<view>
		...
		<view class="content">
			...
			<view class="content-item2">
				...
				<view class="content-item2-son" @click="addGuicheng">
		...
		<view class="" style="overflow: hidden;">
			<view class="mid" hover-class="mid-hover" :data-index="index" v-for="(item,index) in csListArrl" v-if="index <= count" @touchstart="drawStart" @touchmove="drawMove" @touchend="drawEnd" :style="'right:'+item.right+'px'">
				<view class="mid-item1" @click="updateGuicheng(item)">
					<text style="width: 65%;">{{item.czgcmc}}</text>
					<text>{{item.authorname}}</text>
				</view>
				<view class="mid-item3" @click="updateGuicheng(item)">
					<input type="text" v-model="item.fbdw" maxlength="16" disabled=""/>
				</view>
	...
	export default {
		 ...
		 onPullDownRefresh() {
			...
			onReachBottom() {
				...
				setTimeout(function () {
					 ...
					 if(_this.csListArrl.length <= _this.count){
						_this.shanghua = '到底了'
					}
		...
		methods: {
			...
			updateGuicheng(item){
				...
				uni.navigateTo({
					url:'./updataGuiCheng?items='+items
				})
			},
			//显示查询页面
			async serach(){
