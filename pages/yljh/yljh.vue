	...
	<view>
		<u-popup v-model="show" mode="center" width="90%" height="40%" border-radius="14">
			<view class="popup">
				<view class="popup-item">
					<text class="popup-item-text">组织单位：</text>
					<input class="popup-item-input" type="text" v-model="zzdw" />
				</view>
				
				<button type="primary" size="mini" style="width: 50%;margin-left: 25%;margin-top: 80rpx;" @click="serach">确定</button>
			</view>
		...
		<view class="content">
			...
			<view class="content-item2">
				...
				<view class="content-item2-son" @click="addYljh">
		...
		<view class="" style="overflow: hidden;">
			<view class="mid" hover-class="mid-hover" :data-index="index" v-for="(item,index) in csListArrl" v-if="index <= count" @touchstart="drawStart" @touchmove="drawMove" @touchend="drawEnd" :style="'right:'+item.right+'px'">
				<view class="mid-item1" @click="updateYljh(item)">
					<text style="width: 65%;">{{item.zzdw}}</text>
					<text>{{item.ylxs}}</text>
				</view>
				<view class="mid-item2" @click="updateYljh(item)">
					<text>{{item.yjylsj}}</text>
				</view>
				<view class="mid-item3" @click="updateYljh(item)">
					<input type="text" v-model="item.yjylbw" maxlength="16" disabled=""/>
				</view>
