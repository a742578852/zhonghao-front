	...
	<view>
		<u-popup v-model="show" mode="center" width="90%" height="30%" border-radius="14">
			<view class="popup">
				<view class="popup-item">
					<text class="popup-item-text">动火作业级别：</text>
					<input class="popup-item-input" type="text" v-model="dhzyjb" />
				</view>
				<view class="popup-item">
					<text class="popup-item-text">动火所在单位：</text>
					<input class="popup-item-input" type="text" v-model="dhzydw" />
				</view>
			</view>
		...
		<view class="content">
			...
			<view class="content-item2">
				...
				<view class="content-item2-son" @click="addDongHuo">
		...
		<view class="" style="overflow: hidden;">
			<view class="mid" hover-class="mid-hover" :data-index="index" v-for="(item,index) in csListArrl"v-if="index<=count" @touchstart="drawStart" @touchmove="drawMove" @touchend="drawEnd":style="'right:'+item.right+'px'">
				<view class="mid-item1" @click="updataDongHuo(item)">
					<text style="width: 65%;">{{item.dhzyszdw}}</text>
