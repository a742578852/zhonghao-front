	...
	<view>
		<u-popup v-model="show" mode="center" width="90%" height="40%" border-radius="14">
			<view class="popup">
				<view class="popup-item">
					<text class="popup-item-text">标题：</text>
					<input class="popup-item-input" type="text" v-model="bt" />
				</view>
				
				<button type="primary" size="mini" style="width: 50%; margin-left: 25%;margin-top: 90rpx;" @click="serach">确定</button>
			</view>
		...
		<view class="content">
			...
			<view class="content-item2">
				...
				<!-- <view class="content-item2-son" @click="addPxzl">
		...
		<view class="" style="overflow: hidden;">
			<view class="mid" hover-class="mid-hover" :data-index="index" v-for="(item,index) in csListArrl" v-if="index <= count" @touchstart="drawStart" @touchmove="drawMove" @touchend="drawEnd" :style="'right:'+item.right+'px'">
				<view class="mid-item1" @click="updatePxzl(item)">
					<text style="width: 65%;">{{item.bt}}</text>
					<text>{{item.scr}}</text>
				</view>
				<view class="mid-item2" @click="updatePxzl(item)">
					<text>{{item.sslb}}</text>
				</view>
				<view class="mid-item3" @click="updatePxzl(item)">
					<input type="text" v-model="item.gjz" maxlength="16" disabled=""/>
				</view>
